...
serve(async (req) => {
  ...
  try {
    ...
    const { extension_id, admin_key }: RemoveFromQueueRequest =
      await req.json();
    ...
    if (!extension_id) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: "Extension ID is required" }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 400,
        },
    ...
    if (extension.status !== "queued") {
      console.log(
        "❌ Extension is not in queue, current status:",
        extension.status,
      );
      return new Response(
        JSON.stringify({
    ...
    if (
      extension.owner.subscription_status === "free" &&
      extension.owner.exchanges_this_month > 0
    ) {
      console.log("📉 Adjusting monthly exchange count for free tier user...");
      const { error: exchangeError } = await supabase
        .from("users")
        .update({
          exchanges_this_month: Math.max(
            0,
            extension.owner.exchanges_this_month - 1,
          ),
        })
      ...
      if (exchangeError) {
        console.log(
          "❌ Failed to adjust exchange count:",
    ...
    console.log(
      "🎉 Successfully removed extension from queue with full cleanup",
    );
