...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Import Supabase client using the correct syntax for Deno
    ...
    try {
      ...
      console.log(
        "📋 Request body parsed successfully. Raw body:",
    ...
    console.log("🔍 Extracted userId from request body:", userId);
    console.log("🔍 UserId type:", typeof userId);
    console.log("🔍 UserId length:", userId ? userId.length : "undefined/null");

    if (!userId) {
      console.error(
        "❌ Missing userId in request body. RequestBody keys:",
    ...
    try {
      ...
      console.log(
        "✅ JWT verified successfully for user ID length:",
        authUser.id.length,
      );
    } catch (jwtError) {
    ...
    if (authUser.id !== userId) {
      console.log(
        "🚫 Security violation: User attempted to fetch different profile",
      );
      return new Response(
        JSON.stringify({
          success: false,
    ...
    let user;
    try {
      console.log("🔍 Starting database query at:", new Date().toISOString());
      const { data: fetchedUser, error: userError } = await withTimeout(
        supabaseService
          .from("users")
          .select(
            "id, email, name, credit_balance, has_completed_qualification, onboarding_complete, role, subscription_status, exchanges_this_month, last_exchange_reset_date, cookie_preferences, cookie_consent_timestamp, created_at, updated_at",
          )
          .eq("id", userId)
          .single(),
        10000, // 10 seconds (increased from 3s)
        "Database query",
      );
      ...
      if (userError) {
        ...
        if (userError.code === "PGRST116") {
          console.log(
            "ℹ️ User profile not found - this is expected for new users",
          );
          return new Response(
            JSON.stringify({
              success: true,
        ...
        console.log("🔍 Database query returned null for userId:", userId);
        return new Response(
          JSON.stringify({
            success: false,
      ...
      console.log("📋 Fetched user data keys:", Object.keys(user || {}));
      console.log("📋 User onboarding_complete:", user?.onboarding_complete);
      console.log(
        "📋 User has_completed_qualification:",
        user?.has_completed_qualification,
      );
    } catch (dbError) {
    ...
    console.log("🎯 Final response data structure:", {
      ...
      hasCompletedQualification: user?.has_completed_qualification,
    });
    ...
    const totalDuration = Date.now() - startTime;
    console.error(
      "💥 CRITICAL ERROR in fetch-user-profile-for-auth function:",
      {
