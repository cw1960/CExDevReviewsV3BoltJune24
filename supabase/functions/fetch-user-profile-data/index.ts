...
serve(async (req) => {
  ...
  try {
    ...
    const { data: creditTransactions, error: transactionsError } =
      await supabase
    ...
    const { data: reviewRelationships, error: relationshipsError } =
      await supabase
    ...
    const totalReviews =
      reviewAssignments?.filter((a) => a.status === "approved").length || 0;
    const activeReviews =
      reviewAssignments?.filter((a) => a.status === "assigned").length || 0;
