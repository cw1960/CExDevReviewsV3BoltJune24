...
serve(async (req) => {
  ...
  try {
    ...
    const { assignment_id, admin_key, reason }: CancelAssignmentRequest = await req.json()
    ...
    if (!assignment_id) {
      ...
      return new Response(JSON.stringify({ success: false, error: 'Assignment ID is required' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 400
      })
    }

    console.log('✅ Admin authenticated, processing assignment:', assignment_id)
    ...
    if (assignment.status !== 'assigned') {
      console.log('❌ Assignment cannot be cancelled, current status:', assignment.status)
      return new Response(JSON.stringify({ 
        success: false, 
    ...
    if (extensionUpdateError) {
      console.log('❌ Failed to update extension status:', extensionUpdateError.message)
      return new Response(JSON.stringify({ 
        success: false, 
