...
serve(async (req) => {
  ...
  try {
    ...
    const chromeStoreUrlPattern = /^https?:\/\/(chrome\.google\.com\/webstore|chromewebstore\.google\.com)/
    if (!chromeStoreUrlPattern.test(extensionData.chrome_store_url)) {
      console.error('❌ Invalid Chrome Store URL format:', extensionData.chrome_store_url)
      return new Response(
        JSON.stringify({ 
          success: false, 
    ...
    if (createError) {
      ...
      if (createError.code === '23505') {
        userFriendlyError = 'An extension with this Chrome Store URL already exists'
      } else if (createError.code === '23503') {
        userFriendlyError = 'Invalid user ID or foreign key constraint violation'
