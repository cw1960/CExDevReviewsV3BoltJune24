...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log("✅ User found:", {
      id: user.id,
      name: user.name,
      qualified: user.has_completed_qualification,
    });
    ...
    if (!user.has_completed_qualification) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "User must complete qualification before requesting assignments",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 400,
    ...
    if (activeAssignments && activeAssignments.length >= 1) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "You already have an active assignment. Please complete your current review before requesting another.",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 400,
    ...
    console.log(
      "🔍 Step 3: Finding available extensions with Premium Fast Track priority...",
    );
    ...
    if (!availableExtensions || availableExtensions.length === 0) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "No extensions are currently available for review. Please check back later.",
    ...
    const excludedOwnerIds =
      existingRelationships?.map((r) => r.reviewed_owner_id) || [];
    ...
    console.log(
      "✅ Eligible extensions after filtering:",
      eligibleExtensions.length,
    );
    ...
    if (eligibleExtensions.length === 0) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "No new extensions available for review. You have already reviewed extensions from all available developers.",
          silent: silent, // Include silent flag in response so UI knows not to show modal
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const premiumExtensions = eligibleExtensions.filter(
      (ext) => (ext.owner?.subscription_status || "free") === "premium",
    );
    const freeExtensions = eligibleExtensions.filter(
      (ext) => (ext.owner?.subscription_status || "free") === "free",
    );

    console.log(
      `📊 Available extensions: ${premiumExtensions.length} premium, ${freeExtensions.length} free`,
    );
    ...
    if (premiumExtensions.length > 0) {
      ...
      console.log(
        `🎯 Selected PREMIUM extension: ${selectedExtension.name} (Fast Track priority)`,
      );
      ...
      console.log(
        `📋 Selected FREE extension: ${selectedExtension.name} (no premium in queue)`,
      );
    } else {
    ...
    console.log(
      `✅ Final selection: ${selectedExtension.name} (Owner: ${selectedExtension.owner?.subscription_status || "free"}) for reviewer: ${user.name}`,
    );
    ...
    if (extensionUpdateError) {
      ...
      try {
        ...
        await supabase.from("assignment_batches").delete().eq("id", batch.id);
        console.log(
          "🧹 Cleaned up assignment and batch after extension update error",
        );
      } catch (cleanupError) {
        console.error(
          "❌ Failed to cleanup assignment and batch:",
          cleanupError,
        );
      }
      return new Response(
    ...
    console.log(
      `✅ Successfully created assignment #${nextAssignmentNumber} for extension ${selectedExtension.name}`,
    );
    ...
    try {
      ...
      console.error(
        "❌ Failed to trigger MailerLite assignment event:",
        mailerLiteError,
      );
