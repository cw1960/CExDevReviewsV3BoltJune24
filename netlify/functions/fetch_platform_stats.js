...
exports.handler = async function (event, context) {
  ...
  try {
    ...
    const totalFreeUsers = users.filter(
      (u) => u.subscription_status === "free",
    ).length;
    const totalPremiumUsers = users.filter(
      (u) => u.subscription_status === "premium",
    ).length;
    const totalExtensions = extensions.length;
    const totalExtensionsInQueue = extensions.filter(
      (e) => e.status === "queued",
    ).length;
    const totalReviewsAssigned = assignments.length;
    const totalReviewsCompleted = assignments.filter(
      (a) => a.status === "approved" || a.status === "submitted",
    ).length;
    const reviewsInProgress = assignments.filter(
      (a) => a.status === "assigned",
    ).length;
    ...
    const reviewsCompletedLast7Days = assignments.filter(
      (a) =>
        a.status === "approved" &&
        a.submitted_at &&
