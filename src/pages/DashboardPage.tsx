...
export function DashboardPage() {
  const { profile, refreshProfile, updateProfile, isInitialAuthLoading, isProfileRefreshing } = useAuth()
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const { data: extensionsResponse, error: extensionsError } = await withTimeout(
      ...
      if (extensionsResponse?.success) {
        ...
        console.error('❌ Extensions Edge Function returned error:', extensionsResponse.error)
      }
      
      console.log('✅ Extensions fetch completed:', extensionsData.length, 'extensions')

      // Fetch assignments
      console.log('📝 Fetching review assignments...')
      const { data: assignmentsResponse, error: assignmentsError } = await withTimeout(
      ...
      if (assignmentsResponse?.success) {
        ...
        console.error('❌ Assignments Edge Function returned error:', assignmentsResponse.error)
      }
      
      console.log('✅ Assignments fetch completed:', assignmentsData.length, 'assignments')
  ...
  const handleMoveToLibrary = async (extension: Extension) => {
    try {
      ...
      const { data: updateResponse, error } = await supabase.functions.invoke('update-extension-status', {
        body: { 
      ...
      if (error || !updateResponse?.success) {
        throw new Error(updateResponse?.error || error?.message || 'Failed to update extension status')
      }
  ...
  const handleSubmitToQueue = async (extension: Extension) => {
    ...
    try {
      // Use Edge Function to update extension status to avoid RLS issues
      const { data: updateResponse, error: extensionError } = await supabase.functions.invoke('update-extension-status', {
        body: { 
          extension_id: extension.id,
          status: 'queued',
      ...
      if (extensionError || !updateResponse?.success) {
        throw new Error(updateResponse?.error || extensionError?.message || 'Failed to update extension status')
      }
        ...
        <Center h="100vh">
          <Stack align="center" gap="md">
            ...
            <Text size="lg" c="dimmed">
              {isInitialAuthLoading ? 'Loading your account...' : 
               isProfileRefreshing ? 'Refreshing your profile...' : 
               'Loading dashboard...'}
            </Text>
          </Stack>
        </Center>
  ...
  if (!profile) {
    return (
      <Container size="lg">
        <Center h="100vh">
          <Stack align="center" gap="md">
            <Text size="lg" c="red">Profile not found</Text>
