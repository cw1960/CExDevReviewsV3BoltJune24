...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchProfile = useCallback(async (userId: string): Promise<void> => {
    ...
    const fetchProfileWithRetry = async (attempt: number = 1): Promise<void> => {
      const maxAttempts = 5 // Increased attempts for new user signup scenarios
      const baseDelay = 1000 // Increased delay to give database more time
      ...
      try {
        ...
        const { data, error } = await withTimeout(
          ...
          20000 // 20 second timeout for Edge Function (increased from 15s)
        )
        ...
        if (error) {
          ...
          if (attempt < maxAttempts && isRetryableError) {
            ...
            if (attempt >= maxAttempts && isRetryableError) {
              console.log('ℹ️ Max retries reached for profile fetch - this may be a new user whose profile is still being created')
              setProfile(null)
            } else {
              console.error('💥 Non-retryable error - keeping existing profile state')
            }
            return
          }
        }
        ...
        if (!data?.success) {
          ...
          if (errorMsg.includes('not found') || errorMsg.includes('PGRST116')) {
            if (attempt < maxAttempts) {
              ...
              console.log(`⏳ Profile not found (attempt ${attempt}/${maxAttempts}) - retrying in ${delay}ms for potential new user...`)
              await new Promise(resolve => setTimeout(resolve, delay))
              return fetchProfileWithRetry(attempt + 1)
            } else {
              console.log('ℹ️ User profile not found after all retries - this may be a new user whose profile is still being created')
              setProfile(null)
              return
            }
          }
        ...
        if (attempt < maxAttempts) {
          ...
          const isNewUserScenario = error?.message?.includes('not found') || 
                                   error?.message?.includes('PGRST116')
          if (isNewUserScenario) {
            console.log('ℹ️ Max retries reached for new user profile fetch - setting profile to null')
            setProfile(null)
          } else {
  ...
  const signUp = async (email: string, password: string, name: string) => {
    ...
    try {
      ...
      if (!profileData?.success) {
        ...
        throw new Error(`Database error saving new user: ${profileData?.error || 'Unknown error'}`)
      }
  ...
  const updateProfile = async (updates: Partial<UserProfile>) => {
    ...
    console.log('🔄 Current user state:', user ? `user exists (${user.id})` : 'no user');
